            ...
            let text = match args.pop().unwrap() {
                ...
                _ => {
                    return Ok(Value::Constructor {
                        name: "Err".to_string(),
                        args: vec![live_error_value("ui.eventFromJson expects Text")],
                    })
                }
            };
                ...
                Box::pin(async move {
                    ...
                    if req.path == http_path {
                        let html = tokio::task::spawn_blocking(move || {
                            ...
                            let mut runtime = Runtime::new(ctx_for_req.clone(), cancel);
...
fn run_ws_session(
    ...
    loop {
        ...
        let update_applied = runtime.apply(update.clone(), msg_value)?;
        model = runtime.apply(update_applied, model)?;
        ...
        if !ops.is_empty() {
            let payload = serde_json::json!({
                "t": "patch",
                "ops": patch_ops_to_json_value(&Value::List(Arc::new(ops.clone())))?
            });
...
fn live_html_page(title: &str, ws_path: &str, body_html: &str) -> String {
    // Minimal client: event delegation + patch application by `data-aivi-node`.
    format!(
        "<!doctype html>\
...
fn render_vnode_inner(
    ...
    match vnode {
        ...
        other => {
            // Fallback: render unknown nodes as text for debuggability.
            format!(
...
fn style_record_to_text(value: &Value) -> String {
    ...
    // HashMap iteration order is nondeterministic; sort keys for stable HTML output and diffs.
...
fn css_value_to_text(value: &Value) -> String {
    match value {
        ...
        Value::Record(fields) => {
            ...
            crate::runtime::format_value(value)
        }
        other => crate::runtime::format_value(other),
    }
}
...
fn event_id(kind: &str, node_id: &str) -> i64 {
    ...
    // Keep it in signed range for `Int`.
    (hash & 0x7fff_ffff_ffff_ffff) as i64
}

fn diff_vnode(old: &Value, new: &Value, node_id: &str, out: &mut Vec<Value>) {
    // If shapes differ, replace the whole subtree.
    ...
    match (old, new) {
            ...
            if on == "Element" && nn == "Element" && oa.len() == 3 && na.len() == 3 =>
        {
            ...
            // If child segment lists differ, replace (simpler, still correct).
...
fn attrs_to_map(attrs: &Value, node_id: &str) -> HashMap<String, String> {
    ...
    // Reuse the attribute renderer to get canonical text values.
    let s = render_attrs(attrs, node_id, &mut state);
    // Parse ` key="value"` chunks back into a map (safe: all keys are ASCII).
